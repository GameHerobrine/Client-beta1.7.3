    ...
    if (!nbttagcompound.getCompoundTag("Level").hasKey("Blocks")) {
      System.out.println(
          (new StringBuilder())
              ...
              .append(",")
              .append(j)
              .append(" is missing block data, skipping")
              .toString());
      return null;
    }
    Chunk chunk =
        ChunkLoader.loadChunkIntoWorldFromCompound(world, nbttagcompound.getCompoundTag("Level"));
  ...
  public void saveChunk(World world, Chunk chunk) throws IOException {
    ...
    try {
      DataOutputStream dataoutputstream =
          RegionFileCache.getChunkOutputStream(worldDir, chunk.xPosition, chunk.zPosition);
      ...
      worldinfo.setSizeOnDisk(
          worldinfo.getSizeOnDisk()
              + (long) RegionFileCache.getSizeDelta(worldDir, chunk.xPosition, chunk.zPosition));
