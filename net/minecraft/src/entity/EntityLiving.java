...
public abstract class EntityLiving extends Entity {
  ...
  public void playLivingSound() {
    ...
    if (s != null) {
      worldObj.playSoundAtEntity(
          this, s, getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
    }
  }
  ...
  public void spawnExplosionParticle() {
    for (int i = 0; i < 20; i++) {
      ...
      worldObj.spawnParticle(
          ...
          (posZ + (double) (rand.nextFloat() * width * 2.0F)) - (double) width - d2 * d3,
          d,
          d1,
          d2);
    }
    ...
    if (isInWater()) {
      ...
      if (isCollidedHorizontally
          && isOffsetPositionInLiquid(
              motionX, ((motionY + 0.60000002384185791D) - posY) + d, motionZ)) {
      ...
      if (isCollidedHorizontally
          && isOffsetPositionInLiquid(
              motionX, ((motionY + 0.60000002384185791D) - posY) + d1, motionZ)) {
        motionY = 0.30000001192092896D;
      }
    } else {
      float f2 = 0.91F;
  ...
  public void onLivingUpdate() {
    if (newPosRotationIncrements > 0) {
      ...
      rotationPitch +=
          (newRotationPitch - (double) rotationPitch) / (double) newPosRotationIncrements;
