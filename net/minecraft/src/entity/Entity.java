...
import net.minecraft.src.util.nbt.NBTTagDouble;
import net.minecraft.src.util.nbt.NBTTagFloat;
...
public abstract class Entity {
  ...
  public void setPosition(double d, double d1, double d2) {
    ...
    boundingBox.setBounds(
        d - (double) f,
        (d1 - (double) yOffset) + (double) ySize,
        d2 - (double) f,
        d + (double) f,
        (d1 - (double) yOffset) + (double) ySize + (double) f1,
        d2 + (double) f);
  }
  ...
  public void moveEntity(double d, double d1, double d2) {
    ...
    if (flag) {
          ...
          d != 0.0D
              && worldObj
                      .getCollidingBoundingBoxes(
                          this, boundingBox.getOffsetBoundingBox(d, -1D, 0.0D))
                      .size()
                  == 0;
          d5 = d) {
        if (d < d8 && d >= -d8) {
      ...
      for (;
          d2 != 0.0D
              && worldObj
                      .getCollidingBoundingBoxes(
                          this, boundingBox.getOffsetBoundingBox(0.0D, -1D, d2))
                      .size()
                  == 0;
          d7 = d2) {
        if (d2 < d8 && d2 >= -d8) {
    ...
    if (canTriggerWalking() && !flag && ridingEntity == null) {
      distanceWalkedModified +=
          (double) MathHelper.sqrt_double(d10 * d10 + d12 * d12) * 0.59999999999999998D;
      ...
      if (distanceWalkedModified > (float) nextStepDistance && j3 > 0) {
        ...
        if (worldObj.getBlockId(l, j1 + 1, l1) == Block.snow.blockID) {
          ...
          worldObj.playSoundAtEntity(
              this, stepsound.func_1145_d(), stepsound.getVolume() * 0.15F, stepsound.getPitch());
  ...
  public boolean handleWaterMovement() {
    return worldObj.handleMaterialAcceleration(
        boundingBox.expand(0.0D, -0.40000000596046448D, 0.0D).func_28195_e(0.001D, 0.001D, 0.001D),
        Material.water,
        this);
  }
  ...
  public void mountEntity(Entity entity) {
    ...
    if (ridingEntity == entity) {
      ...
      setLocationAndAngles(
          entity.posX,
          entity.boundingBox.minY + (double) entity.height,
          entity.posZ,
          rotationYaw,
          rotationPitch);
      return;
  ...
  public void setPositionAndRotation2(double d, double d1, double d2, float f, float f1, int i) {
    ...
    List list =
        worldObj.getCollidingBoundingBoxes(
            this, boundingBox.func_28195_e(0.03125D, 0.0D, 0.03125D));
    if (list.size() > 0) {
      double d3 = 0.0D;
      for (int j = 0; j < list.size(); j++) {
