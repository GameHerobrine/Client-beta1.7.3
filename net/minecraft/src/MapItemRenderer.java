...
public class MapItemRenderer {
  ...
  public MapItemRenderer(
      FontRenderer fontrenderer, GameSettings gamesettings, RenderEngine renderengine) {
  ...
  public void func_28157_a(EntityPlayer entityplayer, RenderEngine renderengine, MapData mapdata) {
    ...
    tessellator.addVertexWithUV(
        (float) (j + 0) + f, (float) (k + 128) - f, -0.0099999997764825821D, 0.0D, 1.0D);
    tessellator.addVertexWithUV(
        (float) (j + 128) - f, (float) (k + 128) - f, -0.0099999997764825821D, 1.0D, 1.0D);
    tessellator.addVertexWithUV(
        (float) (j + 128) - f, (float) (k + 0) + f, -0.0099999997764825821D, 1.0D, 0.0D);
    tessellator.addVertexWithUV(
        (float) (j + 0) + f, (float) (k + 0) + f, -0.0099999997764825821D, 0.0D, 0.0D);
    tessellator.draw();
    GL11.glEnable(3008 /*GL_ALPHA_TEST*/);
    GL11.glDisable(3042 /*GL_BLEND*/);
    renderengine.bindTexture(renderengine.getTexture("/misc/mapicons.png"));
    for (Iterator iterator = mapdata.field_28173_i.iterator();
        iterator.hasNext();
        GL11.glPopMatrix()) {
      MapCoord mapcoord = (MapCoord) iterator.next();
      GL11.glPushMatrix();
      GL11.glTranslatef(
          (float) j + (float) mapcoord.field_28216_b / 2.0F + 64F,
          (float) k + (float) mapcoord.field_28220_c / 2.0F + 64F,
          -0.02F);
