...
public class GuiTextField extends Gui {
  ...
  public GuiTextField(
      GuiScreen guiscreen, FontRenderer fontrenderer, int i, int j, int k, int l, String s) {
    isFocused = false;
    isEnabled = true;
  ...
  public void textboxKeyTyped(char c, int i) {
    ...
    if (ChatAllowedCharacters.allowedCharacters.indexOf(c) >= 0
        && (text.length() < maxStringLength || maxStringLength == 0)) {
      text += c;
    }
  }
  ...
  public void drawTextBox() {
    ...
    if (isEnabled) {
      ...
      drawString(
          fontRenderer,
          (new StringBuilder()).append(text).append(flag ? "_" : "").toString(),
          xPos + 4,
          yPos + (height - 8) / 2,
          0xe0e0e0);
    } else {
