...
import net.minecraft.src.util.json.*;
...
public class StatFileWriter {
  ...
  public StatFileWriter(Session session, File file) {
    ...
    for (int j = 0; j < i; j++) {
      ...
      if (!file3.exists()) {
        System.out.println(
            (new StringBuilder()).append("Relocating ").append(file2.getName()).toString());
  ...
  public void func_27179_a(Map map) {
    ...
    for (Iterator iterator = map.keySet().iterator();
        iterator.hasNext();
        writeStatToMap(field_25102_a, statbase, ((Integer) map.get(statbase)).intValue())) {
      statbase = (StatBase) iterator.next();
  ...
  public void func_27180_b(Map map) {
    ...
    for (Iterator iterator = map.keySet().iterator();
        iterator.hasNext();
        field_25102_a.put(
            statbase, Integer.valueOf(((Integer) map.get(statbase)).intValue() + i))) {
      statbase = (StatBase) iterator.next();
