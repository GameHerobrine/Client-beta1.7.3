...
public class World implements IBlockAccess {
  ...
  private Set<ChunkCoordIntPair> positionsToUpdate = new HashSet<>();
  ...
  protected void getInitialSpawnLocation() {
    ...
    int j;
    for (j = 0;
        !worldProvider.canCoordinateBeSpawn(i, j);
  ...
  public void setSpawnLocation() {
    ...
    int j;
    for (j = worldInfo.getSpawnZ();
        getFirstUncoveredBlock(i, j) == 0;
    ...
    try {
      ...
      if (chunkProvider instanceof ChunkProviderLoadOrGenerate) {
        ChunkProviderLoadOrGenerate chunkproviderloadorgenerate =
            (ChunkProviderLoadOrGenerate) chunkProvider;
  ...
  public void markBlockNeedsUpdate(int i, int j, int k) {
    for (int l = 0; l < worldAccesses.size(); l++) {
      ((WorldAccess) worldAccesses.get(l)).markBlockAndNeighborsNeedsUpdate(i, j, k);
    }
  }
  ...
  public void markBlockAsNeedsUpdate(int i, int j, int k) {
    for (int l = 0; l < worldAccesses.size(); l++) {
      ((WorldAccess) worldAccesses.get(l)).markBlockRangeNeedsUpdate(i, j, k, i, j, k);
    }
  }

  public void markBlocksDirty(int i, int j, int k, int l, int i1, int j1) {
    for (int k1 = 0; k1 < worldAccesses.size(); k1++) {
      ((WorldAccess) worldAccesses.get(k1)).markBlockRangeNeedsUpdate(i, j, k, l, i1, j1);
    }
  }
  ...
  public void neighborLightPropagationChanged(
      EnumSkyBlock enumskyblock, int i, int j, int k, int l) {
  ...
  public int getSavedLightValue(EnumSkyBlock enumskyblock, int i, int j, int k) {
    j = (int) MathHelper.clamp(j, 0, 127);
    if (i < 0xfe17b800 || k < 0xfe17b800 || i >= 0x1e84800 || k > 0x1e84800) {
  ...
  public void setLightValue(EnumSkyBlock enumskyblock, int i, int j, int k, int l) {
    ...
    for (int i1 = 0; i1 < worldAccesses.size(); i1++) {
      ((WorldAccess) worldAccesses.get(i1)).markBlockAndNeighborsNeedsUpdate(i, j, k);
    }
  }
  ...
  public MovingObjectPosition func_28105_a(Vec3D vec3d, Vec3D vec3d1, boolean flag, boolean flag1) {
    ...
    if ((!flag1 || block == null || block.getCollisionBoundingBoxFromPool(this, l, i1, j1) != null)
      ...
      MovingObjectPosition movingobjectposition =
          block.collisionRayTrace(this, l, i1, j1, vec3d, vec3d1);
    ...
    for (int l1 = 200; l1-- >= 0; ) {
      ...
      if ((!flag1
        ...
        MovingObjectPosition movingobjectposition1 =
            block1.collisionRayTrace(this, l, i1, j1, vec3d, vec3d1);
  ...
  public void playSoundAtEntity(Entity entity, String s, float f, float f1) {
    for (int i = 0; i < worldAccesses.size(); i++) {
      ((WorldAccess) worldAccesses.get(i))
          .playSound(s, entity.posX, entity.posY - (double) entity.yOffset, entity.posZ, f, f1);
    }
  }

  public void playSoundEffect(double d, double d1, double d2, String s, float f, float f1) {
    for (int i = 0; i < worldAccesses.size(); i++) {
      ((WorldAccess) worldAccesses.get(i)).playSound(s, d, d1, d2, f, f1);
    }
  }

  public void playRecord(String s, int i, int j, int k) {
    for (int l = 0; l < worldAccesses.size(); l++) {
      ((WorldAccess) worldAccesses.get(l)).playRecord(s, i, j, k);
    }
  }

  public void spawnParticle(
      String s, double d, double d1, double d2, double d3, double d4, double d5) {
    for (int i = 0; i < worldAccesses.size(); i++) {
      ((WorldAccess) worldAccesses.get(i)).spawnParticle(s, d, d1, d2, d3, d4, d5);
    }
  }
  ...
  protected void obtainEntitySkin(Entity entity) {
    for (int i = 0; i < worldAccesses.size(); i++) {
      ((WorldAccess) worldAccesses.get(i)).obtainEntitySkin(entity);
    }
  }

  protected void releaseEntitySkin(Entity entity) {
    for (int i = 0; i < worldAccesses.size(); i++) {
      ((WorldAccess) worldAccesses.get(i)).releaseEntitySkin(entity);
    }
  }
  ...
  public void addWorldAccess(WorldAccess iworldaccess) {
    worldAccesses.add(iworldaccess);
  }

  public void removeWorldAccess(WorldAccess iworldaccess) {
    ...
    for (int k1 = i; k1 < j; k1++) {
      for (int l1 = i1; l1 < j1; l1++) {
        ...
        for (int i2 = k - 1; i2 < l; i2++) {
          ...
          if (block != null) {
            block.getCollidingBoundingBoxes(
                this, k1, i2, l1, axisalignedbb, collidingBoundingBoxes);
          }
        }
      }
    }
  ...
  public void scheduleBlockUpdate(int i, int j, int k, int l, int i1) {
    ...
    if (scheduledUpdatesAreImmediate) {
      if (checkChunksExist(
          nextticklistentry.xCoord - byte0,
          nextticklistentry.yCoord - byte0,
          nextticklistentry.zCoord - byte0,
          nextticklistentry.xCoord + byte0,
          nextticklistentry.yCoord + byte0,
          nextticklistentry.zCoord + byte0)) {
        int j1 =
            getBlockId(
                nextticklistentry.xCoord, nextticklistentry.yCoord, nextticklistentry.zCoord);
        if (j1 == nextticklistentry.blockID && j1 > 0) {
          Block.blocksList[j1].updateTick(
              this,
      ...
      if (tileentity.func_31006_g()) {
        ...
        if (chunk != null) {
          chunk.removeChunkBlockTileEntity(
              tileentity.xCoord & 0xf, tileentity.yCoord, tileentity.zCoord & 0xf);
        }
      }
    } while (true);
    field_31055_L = false;
  ...
  public void updateEntityWithOptionalForce(Entity entity, boolean flag) {
    ...
    if (!entity.addedToChunk
        || entity.chunkCoordX != k
        || entity.chunkCoordY != l
        || entity.chunkCoordZ != i1) {
      if (entity.addedToChunk && chunkExists(entity.chunkCoordX, entity.chunkCoordZ)) {
        getChunkFromChunkCoords(entity.chunkCoordX, entity.chunkCoordZ)
  ...
  public boolean handleMaterialAcceleration(
      AxisAlignedBB axisalignedbb, Material material, Entity entity) {
    int i = MathHelper.floor_double(axisalignedbb.minX);
    int j = MathHelper.floor_double(axisalignedbb.maxX + 1.0D);
    int k = MathHelper.floor_double(axisalignedbb.minY);
    int l = MathHelper.floor_double(axisalignedbb.maxY + 1.0D);
  ...
  public Explosion newExplosion(
      Entity entity, double d, double d1, double d2, float f, boolean flag) {
  ...
  public boolean updatingLighting() {
    ...
    try {
      int i = 500;
      for (;
          lightingToUpdate.size() > 0;
          ((MetadataChunkBlock) lightingToUpdate.remove(lightingToUpdate.size() - 1))
              .func_4127_a(this)) {
        if (--i <= 0) {
          boolean flag = true;
          return flag;
  ...
  public void scheduleLightingUpdate(
      EnumSkyBlock enumskyblock, int i, int j, int k, int l, int i1, int j1) {
    scheduleLightingUpdate_do(enumskyblock, i, j, k, l, i1, j1, true);
  }

  public void scheduleLightingUpdate_do(
      EnumSkyBlock enumskyblock, int i, int j, int k, int l, int i1, int j1, boolean flag) {
    if (worldProvider.hasNoSky && enumskyblock == EnumSkyBlock.Sky) {
      return;
    }
      ...
      if (flag) {
        ...
        for (int l2 = 0; l2 < j2; l2++) {
          MetadataChunkBlock metadatachunkblock =
              (MetadataChunkBlock) lightingToUpdate.get(lightingToUpdate.size() - l2 - 1);
      ...
      if (lightingToUpdate.size() > 0xf4240) {
        System.out.println(
            (new StringBuilder())
                .append("More than ")
                .append(k2)
                .append(" updates, aborting lighting updates")
                .toString());
  ...
  public void tick() {
    ...
    if (i != skylightSubtracted) {
      ...
      for (int j = 0; j < worldAccesses.size(); j++) {
        ((WorldAccess) worldAccesses.get(j)).updateAllRenderers();
      }
    }
  ...
  protected void updateBlocksAndPlayCaveSounds() {
    ...
    for (Object o : positionsToUpdate) {
      ...
      if (soundCounter == 0) {
        ...
        l3 += i1;
        if (l5 == 0
            && getFullBlockLightValue(l2, l4, l3) <= rand.nextInt(8)
            && getSavedLightValue(EnumSkyBlock.Sky, l2, l4, l3) <= 0) {
          EntityPlayer entityplayer1 =
              getClosestPlayer((double) l2 + 0.5D, (double) l4 + 0.5D, (double) l3 + 0.5D, 8D);
          if (entityplayer1 != null
              && entityplayer1.getDistanceSq(
                      (double) l2 + 0.5D, (double) l4 + 0.5D, (double) l3 + 0.5D)
                  > 4D) {
            playSoundEffect(
                (double) l2 + 0.5D,
                (double) l4 + 0.5D,
      ...
      if (rand.nextInt(16) == 0) {
        ...
        if (getWorldChunkManager().getBiomeGenAt(j3 + k, j4 + i1).getEnableSnow()
            && j5 >= 0
            && j5 < 128
            && chunk.getSavedLightValue(EnumSkyBlock.Block, j3, j5, j4) < 10) {
          int i6 = chunk.getBlockID(j3, j5 - 1, j4);
          int k6 = chunk.getBlockID(j3, j5, j4);
          if (func_27161_C()
              && k6 == 0
              && Block.snow.canPlaceBlockAt(this, j3 + k, j5, j4 + i1)
              && i6 != 0
              && i6 != Block.ice.blockID
              && Block.blocksList[i6].blockMaterial.getIsSolid()) {
  ...
  public boolean TickUpdates(boolean flag) {
    ...
    for (int j = 0; j < i; j++) {
      ...
      int k =
          getBlockId(nextticklistentry.xCoord, nextticklistentry.yCoord, nextticklistentry.zCoord);
      if (k == nextticklistentry.blockID && k > 0) {
        Block.blocksList[k].updateTick(
            this,
            nextticklistentry.xCoord,
            nextticklistentry.yCoord,
            nextticklistentry.zCoord,
  ...
  public List<Entity> getEntitiesWithinAABBExcludingEntity(
      Entity entity, AxisAlignedBB axisalignedbb) {
    field_1012_M.clear();
      ...
      for (int j1 = k; j1 <= l; j1++) {
        if (chunkExists(i1, j1)) {
          getChunkFromChunkCoords(i1, j1)
              .getEntitiesWithinAABBForEntity(entity, axisalignedbb, field_1012_M);
        }
      }
    }
  ...
  public List getEntitiesWithinAABB(Class class1, AxisAlignedBB axisalignedbb) {
    ...
    for (int i1 = i; i1 <= j; i1++) {
      for (int j1 = k; j1 <= l; j1++) {
        if (chunkExists(i1, j1)) {
          getChunkFromChunkCoords(i1, j1)
              .getEntitiesOfTypeWithinAAAB(class1, axisalignedbb, arraylist);
        }
      }
    }
  ...
  public void func_698_b(int i, int j, int k, TileEntity tileentity) {
    ...
    for (int l = 0; l < worldAccesses.size(); l++) {
      ((WorldAccess) worldAccesses.get(l)).doNothingWithTileEntity(i, j, k, tileentity);
    }
  }
  ...
  public ChunkCoordinates getSpawnPoint() {
    return new ChunkCoordinates(
        worldInfo.getSpawnX(), worldInfo.getSpawnY(), worldInfo.getSpawnZ());
  }
  ...
  public float func_27166_f(float f) {
    return (prevThunderingStrength + (thunderingStrength - prevThunderingStrength) * f)
        * func_27162_g(f);
  }
  ...
  public void func_28107_a(EntityPlayer entityplayer, int i, int j, int k, int l, int i1) {
    for (Object worldAccess : worldAccesses) {
      ((WorldAccess) worldAccess).func_28136_a(entityplayer, i, j, k, l, i1);
    }
  }
}
