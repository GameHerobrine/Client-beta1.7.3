...
public class Chunk {
  ...
  public void generateHeightMap() {
    ...
    for (int j = 0; j < 16; j++) {
      for (int k = 0; k < 16; k++) {
        int l = 127;
        for (int i1 = j << 11 | k << 7;
            l > 0 && Block.lightOpacity[blocks[(i1 + l) - 1] & 0xff] == 0;
            l--) {}
  ...
  public void func_1024_c() {
    ...
    for (int j = 0; j < 16; j++) {
      for (int l = 0; l < 16; l++) {
        ...
        int k1;
        for (k1 = j << 11 | l << 7;
            j1 > 0 && Block.lightOpacity[blocks[(k1 + j1) - 1] & 0xff] == 0;
            j1--) {}
  ...
  private void func_1003_g(int i, int j, int k) {
    ...
    for (int j1 = i << 11 | k << 7;
        i1 > 0 && Block.lightOpacity[blocks[(j1 + i1) - 1] & 0xff] == 0;
        i1--) {}
    if (i1 == l) {
      return;
    }
