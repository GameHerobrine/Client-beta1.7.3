...
public class ChunkProviderGenerate implements IChunkProvider {
  ...
  public void generateTerrain(
      int i, int j, byte abyte0[], BiomeGenBase abiomegenbase[], double ad[]) {
    byte byte0 = 4;
    byte byte1 = 64;
    int k = byte0 + 1;
    byte byte2 = 17;
  ...
  public void replaceBlocksForBiome(int i, int j, byte abyte0[], BiomeGenBase abiomegenbase[]) {
    ...
    double d = 0.03125D;
    sandNoise =
        field_909_n.generateNoiseOctaves(sandNoise, i * 16, j * 16, 0.0D, 16, 16, 1, d, d, 1.0D);
    gravelNoise =
        field_909_n.generateNoiseOctaves(
            gravelNoise, i * 16, 109.0134D, j * 16, 16, 1, 16, d, 1.0D, d);
    stoneNoise =
        field_908_o.generateNoiseOctaves(
            stoneNoise, i * 16, j * 16, 0.0D, 16, 16, 1, d * 2D, d * 2D, d * 2D);
    for (int k = 0; k < 16; k++) {
      for (int l = 0; l < 16; l++) {
  ...
  public Chunk provideChunk(int i, int j) {
    ...
    biomesForGeneration =
        worldObj
            ...
            .loadBlockGeneratorData(biomesForGeneration, i * 16, j * 16, 16, 16);
  ...
  private double[] func_4061_a(double ad[], int i, int j, int k, int l, int i1, int j1) {
    ...
    field_4185_d =
        field_910_m.generateNoiseOctaves(
            field_4185_d, i, j, k, l, i1, j1, d / 80D, d1 / 160D, d / 80D);
  ...
  public void populate(IChunkProvider ichunkprovider, int i, int j) {
    ...
    int k4 =
        (int)
            ((mobSpawnerNoise.func_806_a((double) k * d, (double) l * d) / 8D
                    + rand.nextDouble() * 4D
                    + 4D)
                / 3D);
    int l7 = 0;
    ...
    for (int l14 = 0; l14 < byte1; l14++) {
      ...
      (new WorldGenTallGrass(Block.tallGrass.blockID, byte2))
          .generate(worldObj, rand, l19, k22, j24);
    }
    ...
    for (int j19 = k + 8; j19 < k + 8 + 16; j19++) {
      for (int j22 = l + 8; j22 < l + 8 + 16; j22++) {
        ...
        if (d1 < 0.5D
            && k25 > 0
            && k25 < 128
            && worldObj.isAirBlock(j19, k25, j22)
            && worldObj.getBlockMaterial(j19, k25 - 1, j22).getIsSolid()
