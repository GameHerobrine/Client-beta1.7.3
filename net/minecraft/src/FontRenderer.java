...
public class FontRenderer {
  ...
  public FontRenderer(GameSettings gamesettings, String s, RenderEngine renderengine) {
    ...
    for (int i1 = 0; i1 < 256; i1++) {
      ...
      tessellator.addVertexWithUV(
          0.0D, 0.0F + f, 0.0D, (float) l1 / 128F + f1, ((float) k2 + f) / 128F + f2);
      tessellator.addVertexWithUV(
          0.0F + f, 0.0F + f, 0.0D, ((float) l1 + f) / 128F + f1, ((float) k2 + f) / 128F + f2);
      tessellator.addVertexWithUV(
          0.0F + f, 0.0D, 0.0D, ((float) l1 + f) / 128F + f1, (float) k2 / 128F + f2);
  ...
  public void drawStringWithShadow(String s, double i, double j, int k) {
    renderString(s, i + 1, j + 1, k, true);
    drawString(s, i, j, k);
  }

  public void drawString(String s, double i, double j, int k) {
    renderString(s, i, j, k, false);
  }

  public void renderString(String s, double i, double j, int k, boolean flag) {
    if (s == null) {
      return;
    }
    if (flag) {
  ...
  public void func_27278_a(String s, int i, int j, int k, int l) {
    ...
    do {
      ...
      String s1;
      for (s1 = (new StringBuilder()).append(as1[j1++]).append(" ").toString();
          j1 < as1.length
              && getStringWidth((new StringBuilder()).append(s1).append(as1[j1]).toString()) < k;
          s1 = (new StringBuilder()).append(s1).append(as1[j1++]).append(" ").toString()) {}
      int k1;
  ...
  public int func_27277_a(String s, int i) {
    ...
    do {
      ...
      String s1;
      for (s1 = (new StringBuilder()).append(as1[l++]).append(" ").toString();
          l < as1.length
              && getStringWidth((new StringBuilder()).append(s1).append(as1[l]).toString()) < i;
          s1 = (new StringBuilder()).append(s1).append(as1[l++]).append(" ").toString()) {}
      int j1;
