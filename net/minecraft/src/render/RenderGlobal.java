...
public class RenderGlobal implements WorldAccess {
  ...
  private final RenderList[] allRenderLists = {
  ...
  public void loadRenderers() {
    ...
    sortedWorldRenderers =
        new WorldRenderer[renderChunksWide * renderChunksTall * renderChunksDeep];
    int k = 0;
    int l = 0;
    minBlockX = 0;
    minBlockY = 0;
    ...
    for (int j1 = 0; j1 < renderChunksWide; j1++) {
      for (int k1 = 0; k1 < renderChunksTall; k1++) {
        for (int l1 = 0; l1 < renderChunksDeep; l1++) {
          worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide + j1] =
              new WorldRenderer(
                  worldObj, tileEntities, j1 * 16, k1 * 16, l1 * 16, 16, glRenderListBase + k);
          if (occlusionEnabled) {
            worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide + j1].glOcclusionQuery =
                glOcclusionQueryBase.get(l);
          }
          worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide + j1]
          ...
          sortedWorldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide + j1] =
              worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide + j1];
          worldRenderersToUpdate.add(
              worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide + j1]);
          k += 3;
        }
      }
  ...
  public void renderEntities(Vec3D vec3d, ICamera icamera, float f) {
    ...
    TileEntityRenderer.instance.cacheActiveRenderInfo(
        worldObj, renderEngine, mc.fontRenderer, mc.renderViewEntity, f);
    RenderManager.instance.cacheActiveRenderInfo(
        worldObj, renderEngine, mc.fontRenderer, mc.renderViewEntity, mc.gameSettings, f);
    countEntitiesTotal = 0;
    countEntitiesRendered = 0;
    countEntitiesHidden = 0;
    EntityLiving entityliving = mc.renderViewEntity;
    RenderManager.renderPosX =
        ((Entity) (entityliving)).lastTickPosX
            + (((Entity) (entityliving)).posX - ((Entity) (entityliving)).lastTickPosX)
                * (double) f;
    RenderManager.renderPosY =
        ((Entity) (entityliving)).lastTickPosY
    ...
    TileEntityRenderer.staticPlayerY =
        ((Entity) (entityliving)).lastTickPosY
            + (((Entity) (entityliving)).posY - ((Entity) (entityliving)).lastTickPosY)
                * (double) f;
    TileEntityRenderer.staticPlayerZ =
        ((Entity) (entityliving)).lastTickPosZ
            + (((Entity) (entityliving)).posZ - ((Entity) (entityliving)).lastTickPosZ)
                * (double) f;
    ...
    for (int j = 0; j < list.size(); j++) {
      ...
      if (worldObj.blockExists(
          MathHelper.floor_double(entity1.posX), l, MathHelper.floor_double(entity1.posZ))) {
        countEntitiesRendered++;
      ...
      for (int i2 = 0; i2 < renderChunksDeep; i2++) {
        ...
        for (int l2 = 0; l2 < renderChunksTall; l2++) {
          ...
          WorldRenderer worldrenderer =
              worldRenderers[(i2 * renderChunksTall + l2) * renderChunksWide + j1];
    ...
    if (occlusionEnabled && mc.gameSettings.advancedOpengl && !mc.gameSettings.anaglyph && i == 0) {
      ...
      do {
        ...
        for (int k1 = byte0; k1 < i1; k1++) {
          ...
          if (!sortedWorldRenderers[k1].isInFrustum
              || sortedWorldRenderers[k1].isWaitingOnOcclusionQuery) {
            continue;
          }
          float f3 =
              MathHelper.sqrt_float(sortedWorldRenderers[k1].distanceToEntitySquared(entityliving));
          ...
          ARBOcclusionQuery.glBeginQueryARB(
              35092 /*GL_SAMPLES_PASSED_ARB*/, sortedWorldRenderers[k1].glOcclusionQuery);
  ...
  private void checkOcclusionQueryResult(int i, int j) {
    for (int k = i; k < j; k++) {
      ...
      ARBOcclusionQuery.glGetQueryObjectuARB(
          sortedWorldRenderers[k].glOcclusionQuery,
  ...
  private int renderSortedRenderers(int i, int j, int k, double d) {
    ...
    for (int i1 = i; i1 < j; i1++) {
      ...
      if (sortedWorldRenderers[i1].skipRenderPass[k]
          || !sortedWorldRenderers[i1].isInFrustum
    ...
    for (int i2 = 0; i2 < glRenderLists.size(); i2++) {
      ...
      for (int k2 = 0; k2 < k1; k2++) {
        if (allRenderLists[k2].func_862_a(
            worldrenderer.posXMinus, worldrenderer.posYMinus, worldrenderer.posZMinus)) {
          j2 = k2;
        }
      }
      ...
      if (j2 < 0) {
        j2 = k1++;
        allRenderLists[j2].func_861_a(
            worldrenderer.posXMinus, worldrenderer.posYMinus, worldrenderer.posZMinus, d1, d2, d3);
      }
    ...
    for (int l = -byte0 * i; l < byte0 * i; l += byte0) {
      for (int i1 = -byte0 * i; i1 < byte0 * i; i1 += byte0) {
        tessellator.addVertexWithUV(
            l + 0, f9, i1 + byte0, (float) (l + 0) * f6 + f10, (float) (i1 + byte0) * f6 + f11);
        tessellator.addVertexWithUV(
            l + byte0,
            f9,
            i1 + byte0,
            (float) (l + byte0) * f6 + f10,
            (float) (i1 + byte0) * f6 + f11);
        tessellator.addVertexWithUV(
            l + byte0, f9, i1 + 0, (float) (l + byte0) * f6 + f10, (float) (i1 + 0) * f6 + f11);
        tessellator.addVertexWithUV(
            l + 0, f9, i1 + 0, (float) (l + 0) * f6 + f10, (float) (i1 + 0) * f6 + f11);
      }
    }
    ...
    for (int j1 = 0; j1 < l; j1++) {
      ...
      if (!flag) {
        if (worldrenderer1.distanceToEntitySquared(entityliving) > 256F) {
          ...
          for (k2 = 0;
              k2 < byte0
                  && (aworldrenderer[k2] == null
                      || rendersorter.doCompare(aworldrenderer[k2], worldrenderer1) <= 0);
              k2++) {}
          if (--k2 <= 0) {
            continue;
          }
  ...
  public void drawBlockBreaking(
    ...
    if (i == 0) {
      if (damagePartialTime > 0.0F) {
        ...
        double d =
            entityplayer.lastTickPosX
                + (entityplayer.posX - entityplayer.lastTickPosX) * (double) f;
        double d1 =
            entityplayer.lastTickPosY
                + (entityplayer.posY - entityplayer.lastTickPosY) * (double) f;
        double d2 =
            entityplayer.lastTickPosZ
                + (entityplayer.posZ - entityplayer.lastTickPosZ) * (double) f;
        if (block == null) {
          block = Block.stone;
        }
        GL11.glEnable(3008 /*GL_ALPHA_TEST*/);
        ...
        globalRenderBlocks.renderBlockUsingTexture(
            block,
            movingobjectposition.blockX,
            movingobjectposition.blockY,
            movingobjectposition.blockZ,
      ...
      GL11.glColor4f(
          f1, f1, f1, MathHelper.sin((float) System.currentTimeMillis() / 200F) * 0.2F + 0.5F);
    ...
    if (i == 0 && movingobjectposition.typeOfHit == EnumMovingObjectType.TILE) {
      ...
      int j =
          worldObj.getBlockId(
              ...
              movingobjectposition.blockZ);
      if (j > 0) {
        Block.blocksList[j].setBlockBoundsBasedOnState(
            worldObj,
            movingobjectposition.blockX,
            movingobjectposition.blockY,
            movingobjectposition.blockZ);
  ...
  public void playRecord(String s, int i, int j, int k) {
    if (s != null) {
      mc.ingameGUI.setRecordPlayingMessage(
          (new StringBuilder()).append("C418 - ").append(s).toString());
    }
  ...
  public void spawnParticle(
    ...
    if (s.equals("bubble")) {
      ...
      mc.effectRenderer.addEffect(
          new EntityReddustFX(worldObj, d, d1, d2, (float) d3, (float) d4, (float) d5));
