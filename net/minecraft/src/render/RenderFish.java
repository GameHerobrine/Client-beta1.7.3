...
public class RenderFish extends Render {
  ...
  public void func_4011_a(
      EntityFish entityfish, double d, double d1, double d2, float f, float f1) {
    GL11.glPushMatrix();
    ...
    if (entityfish.angler != null) {
      float f9 =
          ((entityfish.angler.prevRotationYaw
                      + (entityfish.angler.rotationYaw - entityfish.angler.prevRotationYaw) * f1)
                  * 3.141593F)
              / 180F;
      ...
      vec3d.rotateAroundX(
          (-(entityfish.angler.prevRotationPitch
                      + (entityfish.angler.rotationPitch - entityfish.angler.prevRotationPitch)
                          * f1)
                  * 3.141593F)
              / 180F);
      vec3d.rotateAroundY(
      ...
      double d7 =
          entityfish.angler.prevPosX
              + (entityfish.angler.posX - entityfish.angler.prevPosX) * (double) f1
              + vec3d.xCoord;
      double d8 =
          entityfish.angler.prevPosY
              + (entityfish.angler.posY - entityfish.angler.prevPosY) * (double) f1
              + vec3d.yCoord;
      double d9 =
          entityfish.angler.prevPosZ
              + (entityfish.angler.posZ - entityfish.angler.prevPosZ) * (double) f1
              + vec3d.zCoord;
      if (renderManager.options.thirdPersonView) {
        float f10 =
            ((entityfish.angler.prevRenderYawOffset
                        + (entityfish.angler.renderYawOffset
                                - entityfish.angler.prevRenderYawOffset)
                            * f1)
                    * 3.141593F)
                / 180F;
      ...
      double d11 =
          entityfish.prevPosY + (entityfish.posY - entityfish.prevPosY) * (double) f1 + 0.25D;
