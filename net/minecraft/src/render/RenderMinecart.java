...
public class RenderMinecart extends Render {
  ...
  public void func_152_a(
      EntityMinecart entityminecart, double d, double d1, double d2, float f, float f1) {
    GL11.glPushMatrix();
    double d3 =
        entityminecart.lastTickPosX
            + (entityminecart.posX - entityminecart.lastTickPosX) * (double) f1;
    double d4 =
        entityminecart.lastTickPosY
            + (entityminecart.posY - entityminecart.lastTickPosY) * (double) f1;
    double d5 =
        entityminecart.lastTickPosZ
            + (entityminecart.posZ - entityminecart.lastTickPosZ) * (double) f1;
    double d6 = 0.30000001192092896D;
    Vec3D vec3d = entityminecart.func_514_g(d3, d4, d5);
    float f2 =
        entityminecart.prevRotationPitch
    ...
    if (f3 > 0.0F) {
      GL11.glRotatef(
          ((MathHelper.sin(f3) * f3 * f4) / 10F) * (float) entityminecart.minecartRockDirection,
          1.0F,
          0.0F,
          0.0F);
    }
    if (entityminecart.minecartType != 0) {
      ...
      if (entityminecart.minecartType == 1) {
        (new RenderBlocks())
            .renderBlockOnInventory(Block.chest, 0, entityminecart.getEntityBrightness(f1));
      } else if (entityminecart.minecartType == 2) {
        (new RenderBlocks())
            .renderBlockOnInventory(Block.stoneOvenIdle, 0, entityminecart.getEntityBrightness(f1));
      }
      GL11.glRotatef(-90F, 0.0F, 1.0F, 0.0F);
      GL11.glTranslatef(0.0F, -0.3125F, 0.0F);
