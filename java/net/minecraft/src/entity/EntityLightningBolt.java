  ...
  public void onUpdate() {
    ...
    if (field_27028_b == 2) {
      worldObj.playSoundEffect(
          posX, posY, posZ, "ambient.weather.thunder", 10000F, 0.8F + rand.nextFloat() * 0.2F);
      worldObj.playSoundEffect(
          posX, posY, posZ, "random.explode", 2.0F, 0.5F + rand.nextFloat() * 0.2F);
    }
    field_27028_b--;
    if (field_27028_b < 0) {
      if (field_27030_c == 0) {
      ...
      List list =
          worldObj.getEntitiesWithinAABBExcludingEntity(
              ...
              AxisAlignedBB.getBoundingBoxFromPool(
                  posX - d, posY - d, posZ - d, posX + d, posY + 6D + d, posZ + d));
