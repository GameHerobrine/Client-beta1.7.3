...
public class EntityWolf extends EntityAnimal {
      ...
      if (entity instanceof EntityPlayer) {
        ...
        if (itemstack != null) {
          if (!isWolfTamed() && itemstack.itemID == Item.bone.shiftedIndex) {
            ...
            looksWithInterest =
                ((ItemFood) Item.itemsList[itemstack.itemID]).getIsWolfsFavoriteMeat();
          }
        }
      }
    }
  ...
  public float getShadingWhileShaking(float f) {
    return 0.75F
        + ((prevTimeWolfIsShaking + (timeWolfIsShaking - prevTimeWolfIsShaking) * f) / 2.0F)
            * 0.25F;
  }

  public float getShakeAngle(float f, float f1) {
    float f2 =
        (prevTimeWolfIsShaking + (timeWolfIsShaking - prevTimeWolfIsShaking) * f + f1) / 1.8F;
    if (f2 < 0.0F) {
      f2 = 0.0F;
    } else if (f2 > 1.0F) {
      f2 = 1.0F;
    ...
    return MathHelper.sin(f2 * 3.141593F)
        * MathHelper.sin(f2 * 3.141593F * 11F)
        * 0.15F
        * 3.141593F;
  }
    ...
    if (super.attackEntityFrom(entity, i)) {
      if (!isWolfTamed() && !isWolfAngry()) {
        ...
        if (isWolfTamed()
            ...
            && ((EntityPlayer) entity).username.equalsIgnoreCase(getWolfOwner())) {
          return true;
        }
        playerToAttack = entity;
      }
