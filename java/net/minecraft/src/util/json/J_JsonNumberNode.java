...
final class J_JsonNumberNode extends J_JsonNode {
  ...
  private static final Pattern field_27226_a =
  ...
  J_JsonNumberNode(String s) {
    ...
    if (!field_27226_a.matcher(s).matches()) {
      throw new IllegalArgumentException(
          (new StringBuilder())
              .append("Attempt to construct a JsonNumber with a String [")
              .append(s)
              .append("] that does not match the JSON number specification.")
              .toString());
    } else {
      field_27225_b = s;
      return;
