...
public class Pathfinder {
  ...
  private PathEntity createEntityPathTo(Entity entity, double d, double d1, double d2, float f) {
    ...
    PathPoint pathpoint1 =
        openPoint(
            MathHelper.floor_double(d - (double) (entity.width / 2.0F)),
      ...
      while (j < i) {
        ...
        if (!pathpoint5.isAssigned() || f1 < pathpoint5.totalPathDistance) {
          ...
          if (pathpoint5.isAssigned()) {
            path.changeDistance(
                pathpoint5, pathpoint5.totalPathDistance + pathpoint5.distanceToNext);
          } else {
  ...
  private int findPathOptions(
    ...
    if (getVerticalOffset(
            entity, pathpoint.xCoord, pathpoint.yCoord + 1, pathpoint.zCoord, pathpoint1)
        == 1) {
      j = 1;
    }
    PathPoint pathpoint3 =
        getSafePoint(
            entity, pathpoint.xCoord, pathpoint.yCoord, pathpoint.zCoord + 1, pathpoint1, j);
    PathPoint pathpoint4 =
        getSafePoint(
            entity, pathpoint.xCoord - 1, pathpoint.yCoord, pathpoint.zCoord, pathpoint1, j);
  ...
  private PathEntity createEntityPath(PathPoint pathpoint, PathPoint pathpoint1) {
    int i = 1;
    for (PathPoint pathpoint2 = pathpoint1;
