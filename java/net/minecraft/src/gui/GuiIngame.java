...
public class GuiIngame extends Gui {
  ...
  public void renderGameOverlay(float f, boolean flag, int i, int j) {
    ...
    if (mc.gameSettings.showDebugInfo) {
      ...
      Render2DEvent event = new Render2DEvent(mc.displayWidth, mc.displayHeight);
    ...
    if (recordPlayingUpFor > 0) {
      ...
      if (j3 > 0) {
        ...
        fontrenderer.drawString(
            recordPlaying, -fontrenderer.getStringWidth(recordPlaying) / 2, -4, k4 + (j3 << 24));
        GL11.glDisable(3042 /*GL_BLEND*/);
        GL11.glPopMatrix();
      }
    }
  ...
  private void renderPumpkinBlur(int i, int j) {
    ...
    GL11.glDisable(3008 /*GL_ALPHA_TEST*/);
    GL11.glBindTexture(
        3553 /*GL_TEXTURE_2D*/, mc.renderEngine.getTexture("%blur%/misc/pumpkinblur.png"));
  ...
  public void addChatMessage(String s) {
    ...
    for (; mc.fontRenderer.getStringWidth(s) > 320; s = s.substring(i)) {
      for (i = 1;
          i < s.length() && mc.fontRenderer.getStringWidth(s.substring(0, i + 1)) <= 320;
          i++) {}
