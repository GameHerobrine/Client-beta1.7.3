...
public class GuiMultiplayer extends GuiScreen {
  ...
  public void initGui() {
    ...
    controlList.add(
        new GuiButton(
            1, width / 2 - 100, height / 4 + 120 + 12, stringtranslate.translateKey("gui.cancel")));
    String s = mc.gameSettings.lastServer.replaceAll("_", ":");
    ((GuiButton) controlList.get(0)).enabled = s.length() > 0;
    field_22111_h =
        new GuiTextField(
            this, fontRenderer, width / 2 - 100, (height / 4 - 10) + 50 + 18, 200, 20, s);
  ...
  protected void actionPerformed(GuiButton guibutton) {
    ...
    if (guibutton.id == 1) {
      ...
      mc.displayGuiScreen(
          new GuiConnecting(mc, as[0], as.length <= 1 ? 25565 : parseIntWithDefault(as[1], 25565)));
    }
  }
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    drawCenteredString(
        fontRenderer,
        stringtranslate.translateKey("multiplayer.title"),
        width / 2,
        (height / 4 - 60) + 20,
        0xffffff);
    drawString(
        fontRenderer,
        stringtranslate.translateKey("multiplayer.info1"),
        width / 2 - 140,
        (height / 4 - 60) + 60 + 0,
        0xa0a0a0);
    drawString(
        fontRenderer,
        stringtranslate.translateKey("multiplayer.info2"),
        width / 2 - 140,
        (height / 4 - 60) + 60 + 9,
        0xa0a0a0);
    drawString(
        fontRenderer,
        stringtranslate.translateKey("multiplayer.ipinfo"),
        width / 2 - 140,
        (height / 4 - 60) + 60 + 36,
        0xa0a0a0);
