...
public class GuiAchievements extends GuiScreen {
  ...
  public GuiAchievements(StatFileWriter statfilewriter) {
    ...
    field_27116_m =
        field_27114_o =
            field_27112_q = AchievementList.openInventory.displayColumn * 24 - c / 2 - 12;
    field_27115_n =
        field_27113_p = field_27111_r = AchievementList.openInventory.displayRow * 24 - c1 / 2;
  }
  ...
  protected void func_27109_b(int i, int j, float f) {
    ...
    for (int l7 = 0; l7 * 16 - i4 < 155; l7++) {
      ...
      for (int i8 = 0; i8 * 16 - j3 < 224; i8++) {
        ...
        drawTexturedModalRect(
            (i2 + i8 * 16) - j3, (j2 + l7 * 16) - i4, k8 % 16 << 4, (k8 >> 4) << 4, 16, 16);
      }
    }
    ...
    for (int l2 = 0; l2 < AchievementList.achievementList.size(); l2++) {
      ...
      char c =
          Math.sin(((double) (System.currentTimeMillis() % 600L) / 600D) * 3.1415926535897931D * 2D)
    ...
    for (int l3 = 0; l3 < AchievementList.achievementList.size(); l3++) {
      ...
      } else if (field_27120_x.func_27181_b(achievement3)) {
        float f2 =
            Math.sin(
                        ((double) (System.currentTimeMillis() % 600L) / 600D)
                            * 3.1415926535897931D
                            * 2D)
                    >= 0.59999999999999998D
                ? 0.8F
      ...
      renderitem.renderItemIntoGUI(
          mc.fontRenderer, mc.renderEngine, achievement3.theItemStack, i6 + 3, k6 + 3);
      GL11.glDisable(2896 /*GL_LIGHTING*/);
      if (!field_27120_x.func_27181_b(achievement3)) {
    ...
    if (achievement != null) {
      ...
      if (field_27120_x.func_27181_b(achievement2)) {
        ...
        if (field_27120_x.hasAchievementUnlocked(achievement2)) {
          fontRenderer.drawStringWithShadow(
              StatCollector.translateToLocal("achievement.taken"), k5, j6 + j7 + 4, 0xff9090ff);
        }
      } else {
        int i7 = Math.max(fontRenderer.getStringWidth(s), 120);
        String s2 =
            StatCollector.translateToLocalFormatted(
                "achievement.requires", new Object[] {achievement2.parentAchievement.statName});
      ...
      fontRenderer.drawStringWithShadow(
          ...
          k5,
          j6,
          field_27120_x.func_27181_b(achievement2)
